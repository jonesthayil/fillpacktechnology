<?php echo '<div class="col col-md-4 col-md-pull-8">
	<div class="service-single-sidebar">
		<div class="services-link-widget widget">
			<ul>
				<li class="dairylink" data-url="/dairy/automatic-auger-filler-with-screw-conveyor.php">
					<a href="/dairy/automatic-auger-filler-with-screw-conveyor.php">Automatic Auger Filler
						Machine</a>
				</li>
				<li class="dairylink" data-url="/dairy/paneer-cutting-machine.php">
					<a href="/dairy/paneer-cutting-machine.php">Paneer cutting machine</a>
				</li>
				<li class="dairylink" data-url="/dairy/linear-conduction-sealing-machine.php">
					<a href="/dairy/linear-conduction-sealing-machine.php">linear conduction sealing
						machine</a>
				</li>
				<li class="dairylink" data-url="/dairy/web-sealing-machine.php">
					<a href="/dairy/web-sealing-machine.php">Web Sealing Machine</a>
				</li>
				<li class="dairylink" data-url="/dairy/automatic-curd-cup-filling-and-sealing-machine.php">
					<a href="/dairy/automatic-curd-cup-filling-and-sealing-machine.php">automatic curd cup
						filling
						and sealing</a>
				</li>
				<li class="dairylink" data-url="/dairy/bottle-washing-machine.php">
					<a href="/dairy/bottle-washing-machine.php">bottle washing machine</a>
				</li>
				<li class="dairylink" data-url="/dairy/vacuum-packing-machine.php">
					<a href="/dairy/vacuum-packing-machine.php">vacuum packing machine</a>
				</li>
				<li class="dairylink" data-url="/dairy/horizontal-flow-pack-packing-machine.php">
					<a href="/dairy/horizontal-flow-pack-packing-machine.php">
						horizontal-flow-pack-packing-machine
					</a>
				</li>
				<li class="dairylink" data-url="/dairy/paneer-pressing-machine.php">
					<a href="/dairy/paneer-pressing-machine.php">paneer pressing machine</a>
				</li>
				<li class="dairylink" data-url="/dairy/tube-filling-machine.php">
					<a href="/dairy/tube-filling-machine.php">tube filling machine</a>
				</li>
				<li class="dairylink" data-url="/dairy/bottle-filling-machine.php">
					<a href="/dairy/bottle-filling-machine.php">Bottle Filling Machine</a>
				</li>
			</ul>
		</div>
		<div class="widget contact-widget">
			<h3>Contact us for help?</h3>
			<div class="details">
				<p>For any kind of query, contact us with the details below.</p>
				<ul>
					<li><i class="fa fa-phone"></i>(+91) 897 657 7735</li>
					<li><i class="fa fa-phone"></i>(+91) 998 778 4735</li>
					<li><i class="fa fa-envelope"></i>export@fillpacktechnology.com</li>
					<li><i class="fa fa-envelope"></i> marketing@fillpacktechnology.com</li>
				</ul>
			</div>
		</div>
	</div>
</div>'; ?>

